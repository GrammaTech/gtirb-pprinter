...
class TestPrintToStdout(unittest.TestCase):
    def test_print_module0(self): 
        output= subprocess.check_output(['gtirb-pprinter','--ir',two_modules_gtirb,'-m','0']).decode(sys.stdout.encoding)
        self.assertTrue('.globl main' in output)
        self.assertFalse('.globl fun' in output)
    
    def test_print_module1(self): 
        output= subprocess.check_output(['gtirb-pprinter','--ir',two_modules_gtirb,'-m','1']).decode(sys.stdout.encoding)
...
class TestPrintToFile(unittest.TestCase):
      def test_print_two_modules(self): 
        output= subprocess.check_output(['gtirb-pprinter','--ir',two_modules_gtirb,'--asm','/tmp/two_modules.s']).decode(sys.stdout.encoding)
