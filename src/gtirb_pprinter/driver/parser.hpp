...
namespace gtirb_multimodule {
    ...
    static const std::string_view module_help_message{
        ...
        "Filenames are specified as MODULE_PATTERN1=FILE_PATTERN1[,MODULE_PATTERN2=FILE_PATTERN2...]."
        "FILE_PATTERN is the name of a path, with one or more selectors. When printing each module,"
        "the selector will be replaced by the appropriate portion of the module name\n"
        ...
        "For example, if the module is named 'libc.so.6', the stem would be 'libc'\n"
        "{ext}: the portion of the module name after the first '.'."
        "For example, if the module is named 'libc.so.6', the extension would be 'so.6'\n"
        "For matching, the {stem} and {ext} selectors can be used together: {stem:lib*}.ext:{so*}"
    };
