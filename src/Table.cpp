...
Table::Table(size_t x) : columns{x} { assert(this->columns > 0); }
...
bool Table::empty() const { return this->table.empty(); }

size_t Table::size() const { return this->table.size(); }

void Table::push_back(std::vector<std::string> x) { this->table.push_back(std::move(x)); }

std::vector<std::vector<std::string>>::iterator Table::begin() { return std::begin(this->table); }

std::vector<std::vector<std::string>>::const_iterator Table::begin() const {
  return std::begin(this->table);
}

std::vector<std::vector<std::string>>::iterator Table::end() { return std::end(this->table); }
